...
logOGcov3 <- lm(log_reco_OG~treat_c+just+ppol+log_perc_OG, data = st.c) # C vs D
```
...
logOGcov3.so <- lm(log_reco_OG~treat_c+just+ppol+log_perc_OG, data = st.c.sinout) # C vs D
```
...
logOGcov3.so.r <- lm_robust(log_reco_OG~treat_c+just+ppol+log_perc_OG, data = st.c.sinout,alpha = 0.05) # C vs D
```
...
barplot01 <- grid.arrange(p1.log,p2.log,p3.log,ncol=3) #Logaritmizado
...
m01 <- st.a.sinout %>% select(COR,log_reco_OG,treat_a,just,ppol,log_perc_OG) %>% na.omit() %>% select(COR,-log_reco_OG,-treat_a,-just,-ppol,-log_perc_OG)
m02 <- st.b.sinout %>% select(COR,log_reco_OG,treat_b,just,ppol,log_perc_OG) %>% na.omit() %>% select(COR,-log_reco_OG,-treat_b,-just,-ppol,-log_perc_OG)
m03 <- st.c.sinout %>% select(COR,log_reco_OG,treat_c,just,ppol,log_perc_OG) %>% na.omit() %>% select(COR,-log_reco_OG,-treat_c,-just,-ppol,-log_perc_OG)
...
barplot02 <- grid.arrange(p1.log,p2.log,p3.log,ncol=3) #Logaritmizado
...
logOGcov1a <- lm_robust(zlog~treat_a+just+ppol+log_perc_OG, data = na.omit(st.a.sinout)) # A vs D | ln(ger/ob) + cov
logOGcov2b <- lm_robust(zlog~treat_b+just+ppol+log_perc_OG, data = na.omit(st.b.sinout)) # B vs D | ln(ger/ob) + cov
logOGcov3c <- lm_robust(zlog~treat_c+just+ppol+log_perc_OG, data = na.omit(st.c.sinout)) # C vs D | ln(ger/ob) + cov
...
htmlreg(
  ...
  custom.coef.names = c(
    ...
    "Ninguno",
    "No sabe",
    "Desigualdad percibida (Log)","Tratamiento","Tratamiento"),
  caption = "",
  caption.above = TRUE,
  reorder.coef = c(2:8, 1),
  no.margin = FALSE,
