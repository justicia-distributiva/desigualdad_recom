...
logOGcov1 <- lm(log_reco_OG~TIPO2+jus2+ppol+log_perc_OG+educ, data = tpad2) #Grupo A vs D |interac
logOGcov2 <- lm(log_reco_OG~TIPO2+jus2+ppol+log_perc_OG+educ, data = tpbd2) #Grupo B vs D |interac
logOGcov3 <- lm(log_reco_OG~TIPO2+jus2+ppol+log_perc_OG+educ, data = tpcd2) #Grupo C vs D |interac

screenreg(l = list(logOGcov1,logOGcov2,logOGcov3))
plot_model(logOGcov1, type = "pred", terms = c("log_perc_OG","educ [1,10]"))


logOGcov4 <- lm(log_reco_OG~treat*jus2+ppol+log_perc_OG+educ, data = st) #Grupo simultáneo | Log(Gerente/obrero) + cov
summary(logOGcov4)
plot_model(logOGcov4, type = "pred", terms = c("jus2","treat"))
...
texreg::screenreg(l = c(ac,bc,cc)) #TEST
...
texreg(list(e1ab03,e1ac03,e2bc03,e1ad04,logOGcov1,e1bd04,logOGcov2,e2cd04,logOGcov3),
       ...
       custom.coef.names = c("Intercepto","Tratamiento","Creencia Mundo Justo",
                                      "Centro", "Derecha", "Ninguno", "No sabe",
                                      "Desigualdad percibida (Log)"),
       caption = "Modelos de regresi\\'on para desigualdad recomendada",
       caption.above = TRUE,
...
texreg(
  ...
  include.rmse = FALSE,
  custom.note = nota,
  file = "C:/Users/JC/Dropbox/Investigacion-Trump/paper/Resultados/tables/anexo_zscore_models.tex")
