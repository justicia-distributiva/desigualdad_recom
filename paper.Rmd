...
```{r barplot2, echo=FALSE, fig.cap = "Resultados del experimento, en base a los valores predichos para la desigualdad recomendada según ocupaciones de alto y bajo estatus.", fig.align='center',fig.height=3}
pacman::p_load(dplyr,ggplot2,grid,gridExtra)
theme_set(new = theme_classic(base_family = "serif"))
...
texreg::texreg(l = list(e2cd04.so.r,logOGcov3.so.r,e1bd04.so.r,logOGcov2.so.r,e1ad04.so.r,logOGcov1.so.r),
        ...
        float.pos = "H", booktabs = TRUE, use.packages = F, 
        include.rs = FALSE, include.rmse = FALSE,include.ci = FALSE,
        custom.note = custom.note)
}
```
...
```{r perc-salarios, echo=FALSE, fig.cap = "Salarios y percepción de desigualdad salarial para un obrero no calificado de una fábrica y Gerente", fig.align='center',out.width='100%',fig.height=4}
pacman::p_load(ggplot2,scales)
load("input/data/proc/st2014sinout.Rdata")

box_salperobr <- 
ggplot(data = st_1,aes(y=salperobr)) + 
...
box_salperger <- 
ggplot(data = st_1,aes(y=salperger)) + 
  stat_boxplot(geom = "errorbar", width=0.3)+
  geom_boxplot(notch = FALSE,width=0.3, outlier.shape = TRUE) +
  guides(fill=FALSE) +
  ggtitle(label ="")+ 
...
box_perc_OG <- 
ggplot(data = st_1,aes(y=perc_OG)) + 
  stat_boxplot(geom = "errorbar", width=0.3)+
  geom_boxplot(notch = FALSE,width=0.3, outlier.shape = TRUE) +
  guides(fill=FALSE) +
  ggtitle(label ="")+ 
...
box_salper <- 
  grid.arrange(box_salperobr,box_salperger,box_perc_OG, nrow = 3) 
```


```{r obgapb, echo=FALSE, fig.cap = "Comparación de la distribución de salario recomendado para un obrero no calificado de una fábrica según asignación del tratamiento.", fig.align='center',out.width='100%',fig.height=4}
pacman::p_load(ggplot2,scales)
load("input/data/proc/st_a-sinout.Rdata")
load("input/data/proc/st_b-sinout.Rdata")
load("input/data/proc/st_c-sinout.Rdata")
...
```{r gergapb, echo=FALSE, fig.cap = "Comparación de la distribución de salario recomendado para un Gerente según asignación del tratamiento.", fig.align='center',out.width='100%',fig.height=4}
pacman::p_load(ggplot2,scales)
load("input/data/proc/st_a-sinout.Rdata")
load("input/data/proc/st_b-sinout.Rdata")
load("input/data/proc/st_c-sinout.Rdata")
...
bal.st.a<- 
  st_1 %>% select(COR,treat_a,log_reco_OG,just,ppol,log_perc_OG,educ) %>% na.omit()
bal.st.b<- 
  st_1 %>% select(COR,treat_b,log_reco_OG,just,ppol,log_perc_OG,educ) %>% na.omit()
bal.st.c<- 
  st_1 %>% select(COR,treat_c,log_reco_OG,just,ppol,log_perc_OG,educ) %>% na.omit()
